...
function card(title) {
  ...
  // Year eyebrow intentionally omitted — the post header already shows the full
  // date (redundant a few inches apart); grid-card date context comes from PostCard.
  return h('div', { style: { position: 'relative', width: 1200, height: 630, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', background: 'radial-gradient(120% 92% at 50% 120%, #2a1c07 0%, #12100a 44%, #0d0b10 72%)', padding: '64px 72px', fontFamily: 'Manrope' } },
    h('div', { style: { display: 'flex', fontFamily: 'Fraunces', fontWeight: 400, fontSize: SIZE, lineHeight: 1.14, color: '#F4EFE7', letterSpacing: -0.5, maxWidth: 1010, lineClamp: 4 } }, primary),
...
async function render(title) {
  const svg = await satori(card(title), { width: 1200, height: 630, fonts: [
...
async function main() {
  ...
  for (const slug of slugs) {
    ...
    fs.writeFileSync(path.join(POSTS, slug, 'cover.png'), await render(title));
    n++;
  }
